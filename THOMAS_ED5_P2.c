...
int ID = 0;
Node *AdicionarUsuario(Lista *lista, char nome[], char rua[], char bairro[], int *numero, int *cep, char *tipo, char usuario_de_fora[], char senha[])
{
    int ID_ATUAL = 0;
...
{
    FILE *fp = fopen("ED5_P2.bin", "rb");
    if (fp == NULL)
    {
        fclose(fp);
        fp = fopen("ED5_P2.bin", "wb");
        Node node = AdicionarUsuario(lista, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    ...
    else
    {
        ...
        while(fp != EOF)
        {
            fread(&usuario, sizeof(Node), 1, fp);
            printf("\nNome: %s", usuario.cadastro.nome);
            AdicionarUsuario(lista, usuario.cadastro.nome, usuario.cadastro.endereco.rua, usuario.cadastro.endereco.bairro, &usuario.cadastro.endereco.numero, &usuario.cadastro.endereco.cep, &usuario.cadastro.tipo, usuario.cadastro.usuario, usuario.cadastro.senha);
        }
...
{
    Node *node;
    for (node = lista->begin; node != NULL; node = node->next)
    {
        printf("\nDEBUG - ENTROU NO FOR");
        if (strcmp(node->cadastro.usuario, usuario) == 0)
        {
           if(strcmp(node->cadastro.senha, senha) == 0)
           {
                printf("\nDEBUG - ACHOU O USUARIO");
                if (node->cadastro.tipo == 'S' || node->cadastro.tipo == 'A')
                    return 1;
                if (node->cadastro.tipo == 'C')
                    return 2;
           }
        }
    }
...
Node *PesquisarNome (Lista *lista, char pesqNome[]) {
    ...
    for (node = lista->begin; node != NULL; node = node->next)
    {
        if (strcmp(node->cadastro.nome, pesqNome) == 0) {
            return node;
        }
    }
    free(node);
...
/*int RetirarAluno(Lista *lista, char usuario[])
{
    if(lista == NULL) return 0;
    if(lista->begin == NULL) return 0;

    Node *prev, *pos;

    for(pos = list->begin; pos != NULL; prev=pos, pos=pos->next)
    {
        if(strcmp(pos->cadastro.usuario == usuario) == 0)
        {
            ...
            else
            {
                if(pos == lista->begin)
                {
                    lista->begin = pos->next;
                }
                else if(pos == lista->end)
                {
                    lista->end = prev;
                    prev->next = NULL;
                }
